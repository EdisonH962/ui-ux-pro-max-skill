...
- 🔦 **Interaction tell**: never ask. Read the product's class off `references/style-bible.md` §Product classes and take the tell in its row — a drone gets the motion tell, a home console the two-beat loop tell with a display already standing on the table, a handheld or phone the one-beat loop, an unpowered product a mechanical hero moment instead of a tell. State the pick in one line while building, along with any extra object that class needs on the table.
